...
class imagePreview {
  ...
  constructor() {
    ...
    this.init = true;
    this.overlayId = uuid();
    this.overlayStyle = 'background-color: rgba(229, 231, 235, .9); position: fixed; top: -100%; left: 0; bottom: -100%; right: 0; opacity: .9; z-index: 66;cursor:pointer;';
  }
  ...
  getImagePosition(dom = null) {
    ...
    const imageRatio = this.getImageResetRatio(imageRealWidth, imageRealHeight);
    const positionY = previewOffset.top + (windowSize.windowHeight - previewOffset.top - previewOffset.bottom) / 2;
    const positionX = previewOffset.left + (windowSize.windowWidth - previewOffset.left - previewOffset.right) / 2;
    // 要进行的移动
  ...
  setPreviewImageElementStyle(dom = null) {
    ...
    i.style = 'transition-property: background-color, border-color, color, fill, stroke, opacity, box-shadow, transform; transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1);transition-duration: 150ms;'
  ...
  setClosedStyle() {
    ...
    // 计算出要应用的变换
    // 已经变换的样式
    const transformArray = document.defaultView.getComputedStyle(this.imageBoxElement).transform.replace('matrix(', '').replace(')', '').replace(' ', '').split(',');
    ...
    // 大图的高度中心点
    const previewCenterTop = previewRect.top + miniRect.height * usedScaleY / 2;
    // 小图左侧中心
    const miniCenterLeft = miniRect.left + miniRect.width / 2;
    // 大图左侧中心点
    const previewCenterLeft = previewRect.left + miniRect.width * usedScaleX / 2;
    const translateX = (miniCenterLeft - previewCenterLeft) + parseFloat(usedTranslateX);
    const translateY = (miniCenterTop - previewCenterTop) + parseFloat(usedTranslateY);
    // 只应用缩放
  ...
  next() {
    this.init = false;
    this.originRect = this.items[this.currentIndex+1].getBoundingClientRect();
    this.setPreviewImageElementStyle()
