...
class ImagePreview {
  ...
  getItems(items) {
    ...
    // 自动取同级元素
    if (items === undefined) {
      const nodes = this.dom.parentElement.children
      for (let i = 0; i < nodes.length; i++) {
        if (nodes[i].dataset.width !== undefined) {
          itemsResult.push(nodes[i])
        }
      }
    } else if (items === false) {
      itemsResult = [this.dom]
    } else {
      itemsResult = items
    }
    return itemsResult
  ...
  setClosedStyle() {
    ...
    setTimeout(() => {
      ...
      this.imageBoxElement.style.opacity = 0
    }, 250)
  }
